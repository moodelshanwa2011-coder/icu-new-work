...
st.set_page_config(page_title="ICU Dashboard Final", layout="wide", initial_sidebar_state="collapsed")

# 2. CSS الاحترافي الشامل (تم تعديل الدوائر لتشمل كل الكلام بالداخل)
st.markdown("""
    <style>
    [data-testid="stAppViewContainer"] { background-color: #000000; color: #ffffff; }
    
    /* 1. كروت المربعات العلوية (كما هي) */
    .kpi-card {
    ...
    .sq-label { color: #aaaaaa; font-size: 28px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 10px; }
    .sq-val { color: #00d4ff; font-size: 60px; font-weight: 900; }
    .sq-bm { color: #444444; font-size: 14px; font-weight: bold; margin-top: 10px; text-transform: uppercase; }

    /* 2. الدوائر المتوهجة (تعديل جذري ليشمل الكلام بالداخل شبه المربع) */
    .circle-container {
        position: relative; width: 280px; height: 280px; border-radius: 50%; /* زيادة الحجم قليلاً ليستوعب النص */
    ...
    .circle-container::after {
        content: ''; position: absolute; background-color: #0a0a0a; inset: 10px; border-radius: 50%;
    }
    
    /* تنسيق النص داخل الدائرة (شبه المربع) */
    .circle-content {
        position: relative; z-index: 10; width: 100%;
        display: flex; flex-direction: column; justify-content: center; align-items: center;
        padding: 20px;
    }
    .cir-label { color: #aaaaaa; font-size: 24px; font-weight: 900; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px; }
    .cir-val { color: #00d4ff; font-size: 55px; font-weight: 900; line-height: 1; margin: 10px 0; }
    .cir-bm { color: #444444; font-size: 13px; font-weight: bold; text-transform: uppercase; margin-top: 5px; }
    ...
    /* تفاصيل الجزء السفلي (مثبتة كما هي) */
...
# 3. قاعدة البيانات (4 أسابيع متغيرة)
...
# الثوابت المرجعية (Benchmarks)
sq_info = [("Falls", 0.18), ("Injuries", 0.04), ("HAPI %", 4.58), ("CLABSI", 3.3), ("CAUTI", 0.4), ("VAP", 2.1)]
# إضافة الـ Benchmark للدوائر أيضاً لعرضه بالداخل
cir_info = [("Restraints", 0.45, 0.9), ("VAE Rate", 1.6, 3.4), ("Turnover", 2.5, 3.0), ("Nurse Hr", 14.5, 12.0), ("RN Edu", 85.0, 70.5), ("C-Diff", 0.0, 0.1)]
        ...
        st.markdown(f"""
            <div class="kpi-card">
                <div class="z-layer">
                    ...
                    <div class="sq-val" style="color:{color}">{val}</div>
                    <div class="sq-bm">BENCHMARK: {bm}</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
# 5. الدوائر العلوية (تم التعديل: كل الكلام بالداخل شبه المربع)
cols2 = st.columns(6)
for i, (name, val, bm) in enumerate(cir_info):
    is_rev = any(x in name for x in ["Hr", "Edu"])
    # تحديد اللون بناءً على الأداء ضد الـ Benchmark
    color = "#00ffaa" if (val >= bm if is_rev else val <= bm) else "#ff4b4b"
    with cols2[i]:
        # HTML الجديد للدوائر: كل المحتوى داخل circle-content
        st.markdown(f"""
            <div class="circle-container">
                <div class="circle-content">
                    <div class="cir-label">{name}</div>
                    <div class="cir-val" style="color:{color}">{val}</div>
                    <div class="cir-bm">BENCHMARK: {bm}</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
# 6. الجزء السفلي (مثبت بكل تفاصيله كما طلبته)
c1, c2 = st.columns([2.2, 1.8])

with c1:
    # مربع الإحصاء (Census)
    st.markdown(f"""<div class="census-box-mini"><div style="color:#FFD700; font-size:12px; font-weight:bold; text-transform:uppercase;">Current Census</div><div class="census-num-mini">{cur['census']}</div></div>""", unsafe_allow_html=True)
    
    # عنوان الأجهزة مع الأسبوع الحالي
...
with c2:
    ...
    fig.update_layout(
        height=450, barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
