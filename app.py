...
# 2. CSS "الأصلي" - تثبيت كامل للمسميات والتنسيق
st.markdown("""
    <style>
    [data-testid="stAppViewContainer"] { background-color: #000000; color: #ffffff; }
    
    /* تنسيق الكروت العلوية (المربعات) */
    .kpi-card {
    ...
    .label-text { color: #aaaaaa; font-size: 20px; font-weight: 900; text-transform: uppercase; margin-bottom: 5px; }
    .val-text { color: #00d4ff; font-size: 45px; font-weight: 900; line-height: 1; }
    .bm-text { color: #444444; font-size: 13px; font-weight: bold; margin-top: 10px; text-transform: uppercase; }

    /* الجزء السفلي (الديفيس) */
...
# داتا السفلي (الديفيس - أسبوعي)
device_weeks = [
    {"week": "Week 1", "census": 28, "vals": [12, 16, 8, 4.0]},
    {"week": "Week 2", "census": 24, "vals": [10, 14, 6, 3.5]}
]
...
# --- العرض العلوي ---
st.markdown(f"<h1 style='text-align: center; color: #00d4ff; font-size: 45px; font-weight:900;'>ICU PERFORMANCE</h1>", unsafe_allow_html=True)
st.markdown(f"<p style='text-align: center; color: #FFD700; font-size: 18px;'>QUARTER: {cur_pdf['q']}</p>", unsafe_allow_html=True)
...
for i in range(6):
    ...
    with c_cir[i]:
        st.markdown(f'<div class="circle-container"><div class="content-box"><div class="label-text" style="font-size:18px;">{cir_names[i]}</div><div class="val-text" style="color:{color}; font-size:42px;">{v}</div><div class="bm-text">BM: {b}</div></div></div>', unsafe_allow_html=True)

st.markdown("<hr style='border-color:#111; margin:40px 0;'>", unsafe_allow_html=True)

# --- العرض السفلي (الديفيس) ---
...
with c1:
    ...
    st.markdown(f'<div class="side-header">ATTACHED DEVICES ({cur_dev["week"]})</div>', unsafe_allow_html=True)
    ...
    for i, (n, m, s) in enumerate(dev_info):
        with g_cols[i]:
            fig = go.Figure(go.Indicator(
                ...
                gauge = {'axis': {'range': [None, m], 'tickvals': []}, 'bar': {'color': "#222"}, 'bgcolor': "#000",
                         'steps': [{'range': [0, s[0]], 'color': "#00ffaa"}, {'range': [s[0], s[1]], 'color': "#FFD700"}, {'range': [s[1], m], 'color': "#ff4b4b"}]}
            ))
